  ...
  const loadAuthStatus = useCallback(async (userOverride = null) => {
    try {
      ...
      if (profile.identity_type === 'alumni') {
        ...
        const { data: approvedList, error: listError } = await supabase
          .from('journal_access_requests')
          .select('status, request_access_start_time, request_access_end_time')
          .eq('requester_id', user.id)
          .eq('status', 'approved')
          .order('created_at', { ascending: false })
          .limit(10);

        if (!listError && Array.isArray(approvedList) && approvedList.length > 0) {
          const hasActive = approvedList.some((req) => {
            if (!req.request_access_start_time || !req.request_access_end_time) return false;
            const startMs = new Date(req.request_access_start_time).getTime();
            const endMs = new Date(req.request_access_end_time).getTime();
